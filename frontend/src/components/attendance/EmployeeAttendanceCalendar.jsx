...
export default function EmployeeAttendanceCalendar({ hideHeader = false }) {
  const { user } = useAuth();
  ...
  const fetchAttendance = async () => {
    ...
    try {
      ...
      // Scope to the logged-in user so attendance stays strictly personal —
      // even though /tasks now returns the whole team's tasks.
      const [taskRes, holidayRes] = await Promise.all([
        api.get(`/tasks?employee_id=${user.id}&start_date=${year}-${mm}-01&end_date=${year}-${mm}-${dd}`),
  ...
  // ── Statistics — only count days up to and including today ──────────────
  const pastAndTodayDays  = days.filter(d => d.day !== null && d.date <= todayStr);
  const holidayCount      = pastAndTodayDays.filter(d => isHoliday(d.date)).length;
  const workingDays       = pastAndTodayDays.length - holidayCount;
  const tasksSubmitted    = pastAndTodayDays.filter(d => !isHoliday(d.date) && statusMap[d.date]).length;
  const tasksMissing      = workingDays - tasksSubmitted;
  const attendancePct     = workingDays > 0 ? Math.round((tasksSubmitted / workingDays) * 100) : 0;
    ...
    <div className="space-y-5">
      ...
      {!hideHeader && (
        <div>
          ...
          <p className="text-sm text-slate-500 dark:text-slate-400 mt-0.5">
            View your monthly attendance based on task submissions
          </p>
        </div>
      )}
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <div className="card p-4 text-center">
          <div className="text-2xl font-bold text-slate-900 dark:text-slate-100">{attendancePct}%</div>
          <div className="text-xs text-slate-500 dark:text-slate-400 font-semibold uppercase tracking-wider">Attendance</div>
        </div>
        <div className="card p-4 text-center">
          <div className="text-2xl font-bold text-green-600 dark:text-green-400">{tasksSubmitted}</div>
          <div className="text-xs text-slate-500 dark:text-slate-400 font-semibold uppercase tracking-wider">Tasks Submitted</div>
        </div>
        <div className="card p-4 text-center">
          <div className="text-2xl font-bold text-red-600 dark:text-red-400">{tasksMissing}</div>
          <div className="text-xs text-slate-500 dark:text-slate-400 font-semibold uppercase tracking-wider">Tasks Missing</div>
        </div>
        <div className="card p-4 text-center">
          <div className="text-2xl font-bold text-yellow-600 dark:text-yellow-400">{holidayCount}</div>
          <div className="text-xs text-slate-500 dark:text-slate-400 font-semibold uppercase tracking-wider">Holidays</div>
        </div>
      </div>
