...
export default function EmployeeTasks() {
  ...
  const loadTasks = useCallback(async () => {
    ...
    setLoading(true);
    try {
      // This page shows only YOUR tasks — team activity lives on the dashboard.
      const r = await api.get(`/tasks?filter=${filter}&employee_id=${user.id}`);
    ...
    <div className="space-y-5">
      <div>
        ...
        <p className="text-sm text-slate-500 dark:text-slate-400 mt-0.5">
          Log your daily task and view your own task history
        </p>
      </div>
